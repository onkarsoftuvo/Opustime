app.controller('UserSettingInfoCtrl', [
  '$scope',
  '$location',
  '$rootScope',
  '$http',
  ...
  '$window',
  'GetWeekDays',
  function($scope, $location, $rootScope, $http, filterFilter, Data, $stateParams, $state, Upload, $translate, $window, GetWeekDays) {
    /*var counter = 0;
    var acounter = 0;*/
    ...
    $scope.remove_practi_refers = function (data) {
      console.log('Number remove: ', data);
    ...
    //   console.log('Here the user data: ', user);
    //   $rootScope.cloading = true;
    ...
    $scope.CreateUser = function (data, file) {
      if (data.practi_info_attributes.appointment_services[0].appointment_type_id == 'N/A') {
        data.practi_info_attributes.appointment_services.shift();
      }
      //$scope.rawuser.practi_info_attributes.appointment_services.shift();
      $rootScope.cloading = true;
      $http.put('/settings/users/' + $stateParams.user_id,{
   ...
   /* $rootScope.deleteuser = function () {
    ...
    // $scope.UpdateUse1r =function(updateData){
    //   console.log(updateData);
    //   $scope.firstname = updateData.first_name;
    //   $http.get('/settings/users/'+$stateParams.user_id+'/edit/'+ {updateData: updateData}).success(function(response){
    //     console.log(response);
    //     $scope.rawuser= response;
    //     console.log($scope.rawuser.first_name);
    //   })
    // }
  }
]);
